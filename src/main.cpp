...
std::string composeYTDLPCommand();
void layoutYTDLPTab();
...
int main(int, char**)
{
	...
	UILy::Window window(UILy::WindowState::WINDOWED, true, "yt-dlp-gui", 860, 500);
	...
	while(window.IsOpen())
	{
		...
		{
			ImGui::DockSpace(ImGui::GetID("Main Dockspace"));
			UILy::endFullWindow();
		}

		if(ImGui::Begin("YTDLP"))
		{
			layoutYTDLPTab();
			ImGui::End();
		}
...
void layoutYTDLPTab()
{
	...
	if(ImGui::Button("Copy Command"))
	{
		...
		ImGui::SetClipboardText(composeYTDLPCommand().c_str());
	}
}
